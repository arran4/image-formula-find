...
func GenerationProcess(worker Required, lastGeneration []*Individual, generation int, newDNA chan string) []*Individual {
	const mutations = 8
	var children = make([]*Individual, 0, len(lastGeneration)*mutations+len(lastGeneration)*len(lastGeneration)+childrenCount*2+1)
	...
	// Local Search / Hill Climbing for the absolute best individual
	// Systematically try point mutations on the top individual to climb the hill quickly.
	if len(lastGeneration) > 0 {
		best := lastGeneration[0]
		dna := best.DNA
		for i := 0; i < len(dna); i++ {
			// Try a few random characters at this position
			for attempt := 0; attempt < 3; attempt++ {
				mut := []byte(dna)
				mut[i] = chars[rand.Intn(len(chars))]
				mStr := string(mut)
				if _, ok := seen[mStr]; !ok && Valid(mStr) {
					seen[mStr] = struct{}{}
					children = append(children, &Individual{
						DNA:             mStr,
						Parent:          []*Individual{best},
						FirstGeneration: generation,
						Lineage:         best.Lineage, // preserve lineage for minor tweaks
					})
				}
			}
		}
	...
	// Increased Crossover (Tournament Selection)
	if len(lastGeneration) > 4 {
		// Perform multiple crossovers instead of just 1
		for c := 0; c < childrenCount*2; c++ {
			// Tournament selection for p1
			t1 := lastGeneration[rand.Intn(len(lastGeneration))]
			t2 := lastGeneration[rand.Intn(len(lastGeneration))]
			p1 := t1
			if t2.Score < t1.Score {
				p1 = t2
			}

			// Tournament selection for p2
			t3 := lastGeneration[rand.Intn(len(lastGeneration))]
			t4 := lastGeneration[rand.Intn(len(lastGeneration))]
			p2 := t3
			if t4.Score < t3.Score {
				p2 = t4
			}
	...
	for len(lastGeneration) < childrenCount && len(children) > 0 {
		...
		// Relax distance check if we don't have enough children to form the next generation
		if minDistance < 10 && len(lastGeneration) + len(children) > childrenCount {
			continue
		}
